...
아래 샘플은 서로 다른 원본 참고 이미지를 사용했습니다. Panel 156이 각 이미지를 독립적으로 한 번만 생성했으며 AI 메타데이터도 제거했습니다. 가로 샘플은 왼쪽 원본·오른쪽 디자인의 정확한 50:50, 세로 샘플은 위 원본·아래 디자인의 정확한 50:50입니다.
...
확인된 한계: 일부 피사체가 크고 배경 생략이 부족합니다. 큰 색면의 판화에 가까워 면과 선의 분리나 어긋난 인쇄가 약한 결과도 있습니다. sample-10의 디자인은 거의 주황색 한 가지 잉크로만 구성되어 엄격한 두 가지 별색 조건을 충족하지 않습니다. 이를 수정하기 위한 추가 생성은 하지 않았습니다.
...
- `design-only`: 전체 캔버스에 Panel 156의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-156`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-156 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-156-prompt.en.md)와 [중국어](references/xxd-panel-156-prompt.zh-CN.md)를 확인하세요.
