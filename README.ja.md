...
以下のサンプルはそれぞれ異なる原画像を使い、Panel 156 が一枚ずつ独立した一回の生成で作成しました。AIメタデータは削除済みです。横長は左に実写、右にデザインを置く厳密な50:50、縦長は上に実写、下にデザインを置く厳密な50:50です。
...
確認された限界：一部は主体が大きく、背景の省略が不足しています。大きな色面の版画に寄り、面と線の分離や版ずれが弱い例もあります。sample-10 のデザイン部分はほぼ橙色の一色刷りで、厳密な二色特色の条件を満たしていません。修正のための追加生成は行っていません。
...
- `design-only`：全画面を Panel 156 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-156` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-156 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-156-prompt.en.md)／[中国語](references/xxd-panel-156-prompt.zh-CN.md)を参照してください。
