...
# XXD Panel 156｜رسم بلونين وانزياح طباعي
...
تستخدم النماذج التالية مراجع أصلية مختلفة. أنشأ Panel 156 كل عمل بصورة مستقلة وفي جولة توليد واحدة، وقد أزيلت البيانات الوصفية للذكاء الاصطناعي. النماذج الأفقية تقسيمها صارم 50:50: الواقع يساراً والتصميم يميناً؛ والنماذج العمودية تقسيمها صارم 50:50: الواقع أعلى والتصميم أسفل.
...
القيود الملحوظة: بعض العناصر الرئيسية أكبر من المطلوب مع إبقاء تفاصيل كثيرة من المشهد؛ وتميل بعض النتائج إلى الطباعة بالكتل مع ضعف فصل الخطوط عن المساحات اللونية أو انزياح التسجيل. يكاد تصميم sample-10 يستخدم حبراً برتقالياً واحداً فقط، فلا يستوفي شرط اللونين الخاصين الصارم. لم تُجرَ محاولات إضافية لتصحيح ذلك.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 156 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-156`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-156 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-156-prompt.en.md) أو [بالصينية](references/xxd-panel-156-prompt.zh-CN.md).
